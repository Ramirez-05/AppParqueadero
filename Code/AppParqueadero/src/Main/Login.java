    ...
    */
    ...
    private void btn_ingresarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ingresarActionPerformed
        ...
        if (!correo.isEmpty() && !contrasena.isEmpty()) {
            ...
            try {
                String iniciarSesion = consumo.consumoGET("http://localhost:8080/iniciarSesion", postData);
                ...
                if (iniciarSesion != null) {
                    ...
                    if (jsonTemp.has("tipo") && jsonTemp.has("estado")) {
                        String tipoUser = jsonTemp.get("tipo").getAsString();
                        String estado = jsonTemp.get("estado").getAsString();
                        String esta = "ACTIVO";
                        if (estado.equalsIgnoreCase(esta)) {
                            String idUsuario = jsonTemp.get("id").getAsString();
                            Map<String, String> obtener = new HashMap<>();
                            obtener.put("id_usuario", idUsuario);
                            ...
                            if (ObtenParking != null) {
                                ...
                                if (success2) {
                                    ...
                                    for (JsonElement parqueadero : parqueaderos) {
                                        ...
                                        id_asignacion = respuesta.get("id_asignacion").getAsString();
                                        System.out.println("ID de Asignación: " + id_asignacion);
                                    }
                                } else {
                                    System.out.println("Error al obtener los parqueaderos.");
                                }
                            ...
                            if (tipoUser.equalsIgnoreCase(ven)) {
                                MainVendedor mainv = new MainVendedor(nit, nombre,direccion,telefono,user,id_asignacion);
                        ...
                        throw new Exception("La respuesta de la API no tiene los campos esperados.");
                    }
                } else {
